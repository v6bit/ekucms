...
function baiduPlayer(){
	...
	if(browser == "Netscape" || browser == "Opera"){
		...
		baiduStr ='<div style="width:'+Player.Width+'px;height:'+Player.Height+'px;overflow:hidden;position:relative;text-align:center; margin:auto;"><iframe src="'+Player.Buffer+'" scrolling="no" width="'+Player.Width+'" height="'+(Player.Height-45)+'" frameborder="0" marginheight="0" marginwidth="0" id="Gxbuffer" style="display:none;position:absolute;z-index:999;text-align:center;margin:auto;"></iframe><object classid="clsid:02E2D748-67F8-48B4-8AB4-0A085374BB99" width="'+Player.Width+'" height="'+Player.Height+'" id="BaiduPlayers" name="BaiduPlayers" onerror="baiduInstall();"><param name="URL" value="'+array["url"]+'"/><param name="NextCacheUrl" value="'+Player.NextCacheUrl+'"><param name="LastWebPage" value="'+Player.LastWebPage+'"><param name="NextWebPage" value="'+Player.NextWebPage+'"><param name="OnPlay" value="onPlay"/><param name="OnPause" value="onPause"/><param name="OnFirstBufferingStart" value="onFirstBufferingStart"/><param name="OnFirstBufferingEnd" value="onFirstBufferingEnd"/><param name="OnPlayBufferingStart" value="onPlayBufferingStart"/><param name="OnPlayBufferingEnd" value="onPlayBufferingEnd"/><param name="OnComplete" value="onComplete"/><param name="Autoplay" value="1"/><param name="ShowStartClient" value="'+Player.ShowStartClient+'"/></object></div>';
		$$("GxPlayer").style.margin = "auto";
		$$("GxPlayer").innerHTML = baiduStr;
...
{
	var playinstall = 'http://union.keatv.com/baidu.html?u='+this.Download+'&v=20130228';
	if (player_down)
	{
		playinstall = player_down;
	}
