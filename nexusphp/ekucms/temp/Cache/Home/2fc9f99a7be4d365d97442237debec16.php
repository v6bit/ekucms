        ...
        <div class="right2">
            ...
            <?php if($stypetagcount > 0): ?><div class="xz1">
                ...
                <div class="xz1">
                	...
                	<span>类型：</span>
                    <p><a href="<?php echo gettypelink($pid);?>" <?php if($pid == 0): ?>class="on3"<?php endif; ?>>全部</a>
                    <?php $tag['name'] = 'menu';$tag['ids'] = ''.$pid.''; $__LIST__ = get_tag_gxmenu($tag); if(is_array($__LIST__)): $i = 0;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): ++$i;$mod = ($i % 2 );?><?php if(is_array($menu["son"])): $i = 0; $__LIST__ = $menu["son"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menuson): ++$i;$mod = ($i % 2 )?><a href="<?php echo gettypelink($menuson['id']);?>" <?php if(($cid)  ==  $menuson["id"]): ?>class="on3"<?php endif; ?> ><?php echo ($menuson["cname"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?><?php endforeach; endif; else: echo "" ;endif;unset($__LIST__);unset($tag);?>
