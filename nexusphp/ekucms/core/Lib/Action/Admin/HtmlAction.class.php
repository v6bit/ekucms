...
class HtmlAction extends AdminAction{	
    ...
    public function videoshow(){
		...
		if ($msid)
		{
			$cidArray		=	get_channel_array($cid);
			$cidList		=	implode(',',$cidArray);
			$cidList		=	$cid.','.$cidList;
		}else{
			if ($arr)
			{
		...
		$start = 1;//开始的页数
		$end = C('url_create_list_num') ? intval(C('url_create_list_num')) : 10; //结束页
		...
		echo '<li>易酷提醒您：正在生成第 '.$start.' - '.$end.' 页，请等待</li>';
		echo '</div>';
		...
		for ($ii = $start; $ii<=$end; $ii++) {
			...
			echo "<script>window.location = '/index.php?s=Admin/Html/createlistwait/page/".$end."/pagecount/".$totalpages."/cid/".$cid."/count/".$count."/cidList/".$cidList."/go/".$go.'/key/'.($key+1)."';</script>";
			//echo'<li>第'.($key+1).'个栏目 第<span>1</span>页 <a href="'.$showurl.'" target="_blank">'.$showurl.'</a> 操作成功</li>';
	...
	public function createlist()
	{
		...
		$list				=	list_search(F('_gxcms/channel'),'id='.$cid);$channel = $list[0];//获取当前栏目的缓存信息
		$start				=	$ii;
		$everycreatenum = $end = C('url_create_list_num') ? intval(C('url_create_list_num')) : 10; //结束页
		$end = $end + $start;
		if ($end > $totalpages) $end = $totalpages;
		...
		*/
		for ($ii = $start; $ii<=$end; $ii++)
		{
			...
			$listdir = str_replace('{!page!}',$ii,get_show_url_dir('video',$cid,$ii));//目录路径
			$showurl = C('webpath').$listdir.C('html_file_suffix');//预览路径	
			$this->buildHtml($listdir,'./','Home:'.$channel['ctpl']);			//print_r($channel);exit;
		}
			//exit($pages);
			//exit('wbf');
	...
	//添加
	public function createlistwait()
	{
		$_GET['pagecount']	=	(int)$_GET['pagecount'];
		$_GET['page'] 		=	(int)$_GET['page'];
		$_GET['count'] 		=	(int)$_GET['count'];
		$_GET['cid'] 		=	(int)$_GET['cid'];
		$_GET['cidList']	=	$_GET['cidList'];
		$_GET['go']			=	intval($_GET['go']);
		$_GET['key']		=	intval($_GET['key']);
		$curpage			=	(int)$_GET['page'] + 1;
		$this->assign('wait',C('url_create_list_time'));
		$this->assign('start',intval($_GET['start']));
		$this->assign('end',intval($_GET['end']));
		
			
		$url			=	'/index.php?s=Admin/Html/createlist/page/'.$curpage.'/pagecount/'.$_GET['pagecount'].'/cid/'.$_GET['cid'].'/count/'.$_GET['count'].'/cidList/'.$_GET['cidList'].'/go/'.$_GET['go'].'/key/'.$_GET['key'];
		if ($curpage <= $_GET['pagecount'])
		{
			$this->assign("pagecount",$_GET['pagecount']);
			$this->assign("page",$_GET['page']);
			$this->assign("cid",$_GET['cid']);
			$this->assign("curpage",$_GET['curpage']);
			$this->assign("url",$url);
			$this->assign("cname",get_channel_name($_GET['cid']));
			$this->display('./views/admin/createlistwait.html');
		}else{
			//echo '进来了<Br>';
			$cidArray	=	explode(',',$_GET['cidList']);
			$cidCount	=	count($cidArray);
			
			$key	=	array_search($_GET['cid'],$cidArray);
			if ($key !== false && $_GET['key'] <= $cidCount)
			{
						$_GET['cid']			=	$cidArray[$key+1];
						$cid					=	$_GET['cid'];
						$list = list_search(F('_gxcms/channel'),'id='.$cid);$channel = $list[0];//获取当前栏目的缓存信息
				...
				if ($_GET['go']) {
					$this->assign("jumpUrl",C('cms_admin').'?s=Admin/Html/Inforead/go/'.$_GET['go']);
					$this->success('恭喜您!影视栏目页已经生成完毕,正在准备生成新闻内容页，请稍等！');
				}else{
					$this->assign("jumpUrl",C('cms_admin').'?s=Admin/Html/Show');
					$this->success('恭喜您!您所选择的栏目页已经生成完毕！');				    
			...
			//echo array_search($_GET['cid'],$cidArray);exit;
    ...
    public function createvideo($array){
		...
		if(C('url_html') && C('url_html_play')){
			if(C('url_html_play') == 1){
				$this->createplay($array,1,false); //只生成一集
			}else{
				$ep = '$$$$$$';
				$playerlist = C('player_list');//多播放器分类
				$play_wbf = explode($ep,$array['playurl']); //播放集数
				
				
				
				foreach($play_wbf as $k1=>$v1)
				{
					$pp = '';
					$sysid = 0;
					$pptype = explode($ep,$array['vodplay']);
										//print_r($play_wbf);
										//print_r($pptype);
					//$playname = $this->getsysplayname($pptype,$k1);
					$playname = $pptype[$k1];
					$playsysid = $this->getsysplaykey($playname);
					//var_dump($playsysid);
					//echo '<Br>';
					
					$vv = explode(chr(13),str_replace(array("\r\n", "\n", "\r"),chr(13),$v1));
					
					foreach($vv as $k2=>$vvv){	
						$this->createplay($array,$k2+1,$playsysid);
					}
				}
	...
	public function createdown($array)
	{
		//print_r($array);
		import("ORG.Io.Dir");
		$dir = new Dir;
		...
		$this->assign($arrays['read']);
		$videodir = get_read_url_dir('video',$arrays['read']['id'],$arrays['read']['cid']);//保存路径
		$videodir = 'down/'.$array['id'];
		$this->buildHtml($videodir,'./','Home:video_down');//生成视频内容页
		$videourl = C('webpath').$videodir.C('html_file_suffix');//预览路径
		echo '<li>'.$array['id'].'的下载页 <a href="'.$videourl.'" target="_blank">'.$videourl.'</a> 操作成功</li>';
		
	}
	
	private function getsysplayname($pptype,$k1)
	{
		echo $k1.':';
		$v = '';
					foreach($pptype as $k3=>$v3){
						if ($k1 == $v3)
						{
							return $v3;
						}
	...
	private function getsysplaykey($name)
	{
		$v = 0;
		$i = 0;
		$playlist = C('player_list');;
					foreach($playlist as $k3=>$v3){
						if ($name == $k3)
						{
							//$pp = $v3;
							return $i;
	...
	//生成视频播放页
    public function createplay($array,$ji,$s){
		$arrays = $this->tags_video_read($array,array($array['id'],$ji,$s));//变量赋值
		$this->assign($arrays['show']);
		$this->assign($arrays['read']);
		$playdir = get_play_url_dir($array['id'],$array['cid'],$ji,$s);//保存路径
