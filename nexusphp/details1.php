...
function douban(doubanid)
{
if(doubanid=="")
alert("请输入正确豆瓣号！");
else
{
var Y="douban.php?id="+"<?php echo $id ?>"+"&action="+doubanid;
//var Y="douban.php?id=";
//var w="<?php echo $id ?>"+"a";
this.location=Y;
//document.getElementById("name").value=doubanid;document.write()
//document.getElementById("doubanid").value=w;
}
...
$res = sql_query("SELECT eku122x_video.cache_stamp, eku122x_video.sp_state, eku122x_video.url, eku122x_video.small_descr, eku122x_video.seeders, eku122x_video.banned, eku122x_video.leechers, eku122x_video.info_hash, eku122x_video.filename, nfo, LENGTH(eku122x_video.nfo) AS nfosz, eku122x_video.last_action, eku122x_video.name, eku122x_video.owner, eku122x_video.save_as, eku122x_video.descr, eku122x_video.visible, eku122x_video.size, eku122x_video.added, eku122x_video.views, eku122x_video.hits, eku122x_video.times_completed, eku122x_video.id, eku122x_video.type, eku122x_video.numfiles, eku122x_video.anonymous, categories.name AS cat_name, sources.name AS source_name, media.name AS medium_name, codecs.name AS codec_name, standards.name AS standard_name, processings.name AS processing_name, teams.name AS team_name, audiocodecs.name AS audiocodec_name FROM eku122x_video LEFT JOIN categories ON eku122x_video.category = categories.id LEFT JOIN sources ON eku122x_video.source = sources.id LEFT JOIN media ON eku122x_video.medium = media.id LEFT JOIN codecs ON eku122x_video.codec = codecs.id LEFT JOIN standards ON eku122x_video.standard = standards.id LEFT JOIN processings ON eku122x_video.processing = processings.id LEFT JOIN teams ON eku122x_video.team = teams.id LEFT JOIN audiocodecs ON eku122x_video.audiocodec = audiocodecs.id WHERE eku122x_video.id = $id LIMIT 1")
or sqlerr();
...
elseif ($row['banned'] == 'yes' && get_user_class() < $seebanned_class)
	...
	if ($_GET["hit"]) {
		sql_query("UPDATE eku122x_video SET views = views + 1 WHERE id = $id");
	}
	...
	if (!isset($_GET["cmtpage"])) {
		...
		//$s=htmlspecialchars($row["name"]).($sp_torrent ? "&nbsp;&nbsp;&nbsp;".$sp_torrent : "");
                $s=htmlspecialchars($torrentnameprefix ."." .$row["save_as"]);
...
//------------------
		...
		$torrentdetailad=$Advertisement->get_ad('torrentdetail');
               ...
               $fdescrraw=strip_tags(htmlspecialchars_decode($row["descr"]));
               $fdescr=str_replace("&nbsp;"," ",$fdescrraw);

                $myfp=fopen("myfile.txt","w");
                fwrite($myfp,$fdescr);
                fwrite($myfp,"\n\n\n");
                fwrite($myfp,$row["descr"]);
                fclose($myfp);
//                $fdescr=$row["descr"];
	...
	if ($imdb_id && $showextinfo['imdb'] == 'yes' && $CURUSER['showimdb'] != 'no')
	{
				...
				case "1" :
					{
						...
						if($similiar_movies)
						{
							...
							foreach($similiar_movies as $similiar_movies_each)
							{
								...
								{
									$similiar_res = sql_query("SELECT id FROM eku122x_video WHERE url = " . sqlesc((int)$imdb_id_new) . " AND id != ".sqlesc($id)." ORDER BY RAND() LIMIT 1") or sqlerr(__FILE__, __LINE__);
		...
		if ($imdb_id)
		{
			$where_area = " url = " . sqlesc((int)$imdb_id) ." AND eku122x_video.id != ".sqlesc($id);
			$copies_res = sql_query("SELECT eku122x_video.id, eku122x_video.name, eku122x_video.sp_state, eku122x_video.size, eku122x_video.added, eku122x_video.seeders, eku122x_video.leechers, categories.id AS catid, categories.name AS catname, categories.image AS catimage, sources.name AS source_name, media.name AS medium_name, codecs.name AS codec_name, standards.name AS standard_name, processings.name AS processing_name FROM eku122x_video LEFT JOIN categories ON eku122x_video.category=categories.id LEFT JOIN sources ON eku122x_video.source = sources.id LEFT JOIN media ON eku122x_video.medium = media.id  LEFT JOIN codecs ON eku122x_video.codec = codecs.id LEFT JOIN standards ON eku122x_video.standard = standards.id LEFT JOIN processings ON eku122x_video.processing = processings.id WHERE " . $where_area . " ORDER BY eku122x_video.id DESC") or sqlerr(__FILE__, __LINE__);
