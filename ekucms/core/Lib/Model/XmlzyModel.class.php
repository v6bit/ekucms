...
class XmlzyModel extends Model {	
    ...
    public function ziyuan_a($array_url){
		...
		if ($xml) {
			...
			if($array_url['action']){
				...
				preg_match_all('/<video><last>([\s\S]*?)<\/last><id>([0-9]+)<\/id><tid>([0-9]+)<\/tid><name><\!\[CDATA\[([\s\S]*?)\]\]><\/name><type>([\s\S]*?)<\/type><dt>([\s\S]*?)<\/dt><note><\!\[CDATA\[([\s\S]*?)\]\]><\/note>/',$xml,$array_vod);
			}
			//组合数据
	...
	public function ziyuan_feifei($array_url){
		$array_tpl['httpurl'] = $array_url['xmlurl'].'/index.php?s=plus-api-xml-cms-ff-action-'.$array_url['action'].'-vodids-'.$array_url['vodids'].'-cid-'.$array_url['cid'].'-wd-'.urlencode($array_url['wd']).'-inputer-'.$array_url['inputer'].'-play-'.$array_url['play'].'-h-'.$array_url['h'].'-p-'.$array_url['p'];
		$array_tpl['httpurl'] = str_replace('@','/',$array_tpl['httpurl']);//还原目标网址
		$xml = get_collect_file($array_tpl['httpurl']);
		if ($xml) {
			return $this->ff_gx_xml($array_url,$xml);
	...
	public function xml_update($vod,$array,$must=false){
		if('gxcms' == $array['inputer']){
			...
			//判断百度影音更新开始
			$vodplayarr	=	explode('$$$$$$',$array['vodplay']);
			$kk = false;
			foreach($vodplayarr as $k=>$v)
			{
				if ($v == 'baidu')
				{
					$kk = $k;
			...
			$playurlarr	=	explode('$$$$$$',$array['playurl']);
			$baiduplayurl = $kk !== false ? $playurlarr[$kk] : false;
			//判断百度影音更新结束
		...
		if(!$must){//是否强制更新资料
			...
			//var_dump($kk);exit;
			if ($kk !== false) //有此类型的播放地址
			{
				if ($baiduplayurl == $vod['playurl']) {
					return '播放地址未变化，退出更新！';
				}
				$count_vod   = count(explode(chr(13),($vod['playurl'])));
				$count_array = count(explode(chr(13),trim($baiduplayurl)));
		...
		if ($kk !== false) //有原地址则当前更新
		{
			$playurlarr[$kk]	=	$vod['playurl'];
		}else{ //没有源地址则新加播放器地址
			$playurlarr[count($playurlarr)] = $vod['playurl'];
			$edit['vodplay']	=	$array['vodplay'].'$$$$$$baidu';
		}
		$newplayurl 		=	implode('$$$$$$',$playurlarr);
		
		$edit['intro']    = $vod['intro'];
		$edit['cid']     = $vod['cid'];
		if (intval($vod['serial']) > intval($array['serial']))
		{
			$edit['serial']  = $vod['serial'];
		}
		//$edit['playurl'] = $vod['playurl'];
		$edit['playurl'] = $newplayurl;
		$edit['addtime'] = time();
		$edit['reurl']   = $vod['reurl'];
